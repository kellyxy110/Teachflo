...
type BookshelfDocument = BookshelfDocumentRecord & { createdAt: string };
const formatType = bookshelfFileType;
const formatDate = (value: string) => new Date(value).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" });
...
function SourceRow({ document }: { document: BookshelfDocument }) {
  ...
  return <li className="px-4 py-3 hover:bg-bg/60"><div className="grid grid-cols-1 md:grid-cols-[minmax(0,2fr)_1fr_1fr_1fr_1fr] gap-2 md:gap-4 items-center"><div className="min-w-0 flex items-start gap-3"><FileText size={18} className="text-primary shrink-0 mt-0.5" aria-hidden="true" /><div className="min-w-0"><Link href={`/library/bookshelf/${document.id}`} className="font-medium text-text break-words hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">{document.title}</Link><p className="text-xs text-muted break-all">{document.fileName}</p><div className="flex flex-wrap gap-2 mt-1 text-xs text-muted"><span>{document.classLevel ?? "Any level"}</span>{document.pageCount ? <span>{document.pageCount} pages</span> : null}<span className="inline-flex items-center gap-1"><LockKeyhole size={11} aria-hidden="true" />Private</span></div></div></div><span className="text-sm text-text-2">{document.subject}</span><span className="text-sm text-text-2">{formatType(document.mimeType, document.fileName)}</span><span className={`inline-flex items-center gap-1 text-xs font-medium ${info.className}`} aria-label={info.label}><Icon size={13} className={document.status === "PROCESSING" ? "animate-spin" : ""} aria-hidden="true" />{info.label}</span><time className="text-sm text-muted" dateTime={document.createdAt}>{formatDate(document.createdAt)}</time></div>{document.status === "FAILED" && <p className="text-xs text-danger mt-2 ml-8">{document.error?.includes("OCR") ? "Readable text could not be extracted; OCR is not currently available." : document.error ?? "Processing failed. Your original source remains preserved."}</p>}</li>;
}
