...
import { generateBroadsheetBuffer } from "@/lib/services/reports/broadsheet";

export const maxDuration = 60;

// GET /api/student-hub/reports/broadsheet?classId=&term=&session=
export async function GET(request: Request) {
  const auth = await safeAuth();
  if (!auth.userId) return Response.json({ error: "Unauthorized" }, { status: 401 });

  const { ok } = await rateLimit(`report-broadsheet:${auth.userId}`);
  ...
  const buffer = generateBroadsheetBuffer(analytics);
